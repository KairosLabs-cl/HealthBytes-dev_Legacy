...
class TestGetCart:
    ...
    def test_get_cart_with_items(self, client, db_session, test_user, test_products, auth_headers):
        ...
        cart_item = CartItem(user_id=test_user.id, product_id=test_products[0].id, quantity=2)
...
class TestAddToCart:
    ...
    def test_add_new_item(self, client, test_products, auth_headers):
        ...
        payload = {"product_id": test_products[0].id, "quantity": 2}
    ...
    def test_add_existing_item_increments_quantity(
        self, client, db_session, test_user, test_products, auth_headers
    ):
        """Should increment quantity when adding existing item"""
        # Add item first
        cart_item = CartItem(user_id=test_user.id, product_id=test_products[0].id, quantity=1)
        db_session.add(cart_item)
        db_session.commit()
...
class TestUpdateCartItem:
    ...
    def test_update_item_quantity(self, client, db_session, test_user, test_products, auth_headers):
        ...
        payload = {"quantity": 5}
        response = client.put(
            f"/cart/items/{test_products[0].id}", json=payload, headers=auth_headers
        )
...
class TestRemoveFromCart:
    ...
    def test_remove_item(self, client, db_session, test_user, test_products, auth_headers):
        ...
        # Remove it
        response = client.delete(f"/cart/items/{test_products[0].id}", headers=auth_headers)
...
class TestMergeCart:
    ...
    def test_merge_with_existing_items(
        ...
        cart_item = CartItem(user_id=test_user.id, product_id=test_products[0].id, quantity=3)
        db_session.add(cart_item)
        db_session.commit()
        ...
        product1_item = next(
            item for item in data["items"] if item["product_id"] == test_products[0].id
        )
