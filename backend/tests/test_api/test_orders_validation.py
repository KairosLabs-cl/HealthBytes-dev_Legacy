...
async def test_order_uses_database_price_not_client_price(
    ...
    response = client.post(
        "/orders/", json=order_data, headers={"Authorization": f"Bearer test_token"}  # Mock token
    )

    # Verificar que la orden se creó (puede ser 201 o fallar con 401 por auth)
    # Si falla autenticación es esperado en este test
    if response.status_code == 201:
        result = response.json()
        # Verificar que se usó precio real, no cliente
        # (Este validación depende de cómo calcule total)
    assert len(result.items) == 1

    # CRÍTICO: Verificar que se usó el precio REAL, no el falso
    assert (
        result.items[0].price == real_price
    ), f"❌ INSEGURO: Se usó precio {result.items[0].price} en lugar de {real_price}"

    assert (
        result.items[0].price != client_price
    ), f"❌ CRÍTICO: Se aceptó precio falso del cliente: {client_price}"

    print(f"✅ SEGURO: Orden creada con precio correcto ${real_price}")


@pytest.mark.skip(reason="Requires proper fixture setup with auth mocking")
@pytest.mark.asyncio
async def test_order_rejects_nonexistent_product(test_user: User):
    """
    ...
    order_data = OrderCreate(
        order={}, items=[OrderItemCreate(productId=99999, quantity=1, price=100)]  # No existe
    )
    ...
    with pytest.raises(HTTPException) as exc_info:
        await create_order(order_data, db_session, test_user)
...
async def test_order_multiple_items_validation(
    ...
    result = await create_order(order_data, db_session, test_user)

    # Verificar que TODOS los items tienen precios reales
    assert result.items[0].price == test_product.price  # 10.99, no 999
    assert result.items[1].price == product2.price  # 20.00, no 888
    assert result.items[2].price == product3.price  # 15.50, no 777
