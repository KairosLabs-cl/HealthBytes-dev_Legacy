...
async def test_order_creation_query_count(db_session, test_user_perf, item_count):
    ...
    # Create order data
    order_items = [
        OrderItemCreate(product_id=pid, quantity=1, price=10.0)
    ...
    for idx, q in enumerate(counter.queries, 1):
        query_type = 'SELECT' if 'SELECT' in q.upper() else 'INSERT' if 'INSERT' in q.upper() else 'OTHER'
    ...
    print(f"   Product queries: {product_select_count} (constant, regardless of {item_count} items)")
...
async def test_verify_single_product_query(db_session, test_user_perf):
    ...
    product_selects = [
        ...
        if 'SELECT' in q.upper() and 'product' in q.lower()
    ]
    ...
    assert 'IN' in product_query.upper(), (
        "Product query should use IN clause for batch fetching"
    )
