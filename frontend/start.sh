...
ensure_frontend_env() {
  ...
  if [ -f "$SETUP_SCRIPT" ]; then
    clerk_key="$(awk -F= '/^DEFAULT_CLERK_KEY=/{gsub(/"/, "", $2); print $2; exit}' "$SETUP_SCRIPT")"
  fi
  ...
  if [ -z "${clerk_key:-}" ]; then
    ...
    echo "   Code: ENV_ERROR"
    echo "   Details:"
    echo "     - Expected EXPO_PUBLIC_CLERK_PUBLISHABLE_KEY in frontend/.env"
    echo "     - Action: Run ./setup-env.sh and then pnpm start"
    exit 1
  fi

  cat > "$ENV_FILE" <<EOF
# HealthBytes Frontend Environment Variables
# Auto-generated by start.sh for local development
...
EOF
  ...
  echo "✅ Created frontend/.env with localhost API URL and Clerk publishable key."
  echo "   Run ./setup-env.sh later if you need Expo Go on a physical device."
}
