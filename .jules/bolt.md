...
## 2024-03-24 - Prevent unnecessary re-renders in FlatList items by returning primitive values from Zustand selectors
**Learning:** In Zustand, returning an entire Set from a selector (e.g., `useCart(state => state.updatingProducts)`) triggers a component re-render every time the Set's reference changes, even if the change does not affect the specific item the component cares about. This severely impacts performance in large lists like shopping carts.
**Action:** Always use highly specific selectors that return primitive values, such as the boolean result of a `.has()` check for a specific item (e.g., `const isUpdating = useCart(state => state.updatingProducts.has(item.id))`).
