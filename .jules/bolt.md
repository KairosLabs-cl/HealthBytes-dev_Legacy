...
## 2026-04-16 - Prevent Unnecessary Re-renders with Zustand Selectors
...
**Action:** Use inline selectors to access specific array elements or primitive properties (e.g., `useCart((state) => state.items.find(i => i.product.id.toString() === id)?.quantity || 0)`).
