...
## 2024-03-19 - Prevent unnecessary re-renders in list items by avoiding returning entire Sets from Zustand selectors
**Learning:** Returning an entire Set from a selector (e.g., `useCart(state => state.updatingProducts)`) triggers a component re-render every time the Set's reference changes, even if the change does not affect the specific item the component cares about. This causes O(N) re-renders in list components when a single item is modified.
