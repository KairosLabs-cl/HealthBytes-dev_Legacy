...
## 2026-03-16 - Timing Attack and Enumeration in Auth Service
**Vulnerability:** The `login_user` function in `auth_service.py` returned immediately when a user was not found, creating a timing attack vector. Additionally, it did not check for `None` passwords for OAuth users, risking an `AttributeError`.
**Learning:** Security mitigations (like dummy password verification) must be applied consistently across all authentication entry points, not just the primary API route.
**Prevention:** Use consistent authentication verification flows that include timing attack mitigations and null-checks for OAuth passwords in all service layer authentication functions.
